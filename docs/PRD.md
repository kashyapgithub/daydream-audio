...
### 6.17 Extreme Echo Range & Vari-Speed Tape Slowdown

Direct follow-up request after 6.15: even the widened echo/reverb range still felt too tame for genuinely heavy, drenched effects ("a loooot of echo"), and a real tape/vinyl-style slowdown was explicitly requested alongside it — this combination (heavy trailing echo + pitch-dropping slowdown) is a well-known effect pairing (the "chopped & screwed" / vaporwave aesthetic), so both were addressed together:

- **Echo pushed further:** delay time ceiling raised from 2000ms to 3000ms, feedback ceiling raised from 92% to 96%, wet-mix ceiling raised again (dry can drop to ~8%, wet boosted up to 2.4x) so echo can become the dominant or effectively only audible signal at max settings.
- **Reverb pushed to match:** wet-mix ceiling raised (dry down to ~10%, wet up to 2.6x), so both time-based effects scale consistently.
- **Vari-Speed:** playback speed range widened from 0.5x–1.5x to 0.25x–2.0x, with pitch now dropping with tempo by default (`AudioTrack.PlaybackParams.setPitch()` follows `setSpeed()`) — real analog tape/vinyl slowdown character, rather than the pitch-corrected "podcast speed control" behavior the app previously shipped with. A **Preserve Pitch** toggle is available for the old pitch-corrected behavior.
- **Honesty verification layer (important addition, not just a range widening):** when this was first implemented, direct questioning surfaced a real gap — Android's own `PlaybackParams` documentation states that out-of-range speed/pitch values are handled by an OEM-dependent "fallback mode" that may silently clamp or mute rather than throw, and the original code wrapped the whole call in a bare `catch (Exception) {}` with zero logging, meaning a silent failure on some device would have been indistinguishable from success. This was corrected: the engine now reads back `AudioTrack.getPlaybackParams().speed` immediately after every request and compares it to what was actually asked for, exposing a `speedAppliedAsRequested` flag and the confirmed value to the UI. If a device clamps an extreme request, Advanced Mode now tells the user plainly ("Your device applied 0.4x instead of 0.25x — this is a hardware/OEM limit, not a bug") instead of leaving them to wonder why the slider didn't do what it said. This is the same pattern already established for the session-0 system-wide hook (12.1) — verify and disclose, never assume and stay silent.
...
**Critical capability boundary, discovered after building 6.15/6.17 (documenting so it isn't rediscovered the hard way):** the advanced time/space effects built for the in-app Freeverb/echo engine — Room Size, Wall Material, the extreme echo/reverb range, and Vari-Speed tempo — do **not** reach System-Wide Mode at all, for two different reasons:

- **Reverb:** System-Wide Mode drives Android's *stock* `EnvironmentalReverb`/`PresetReverb` effects (the only reverb types attachable to another app's session), through a separate, older parameter set keyed on a `spatialRoomType` string ("Intimate Studio", "Concert Hall", "Cathedral", "Natural") — a different naming scheme from the RoomSize/WallMaterial enums in 6.15, and hard-capped by the platform (e.g. reflection delay clamped to 0–300ms) well short of what the custom Freeverb engine can now do. There is currently no Wall Material equivalent in this path at all.
- **Vari-Speed / Tempo:** this is not a missing feature but a hard Android platform limit — there is no `AudioEffect` subclass for playback speed, and `AudioTrack.PlaybackParams` only works on an `AudioTrack` the app itself created. **There is no way to change the playback speed of another app's live audio stream on unrooted Android.** Tempo/Vari-Speed is therefore permanently in-app-player-only, the same category of hard boundary as the `MediaProjection` live-capture exclusion already documented in section 23.

**Product implication:** the app has two meaningfully different capability tiers depending on playback source, and this should be surfaced to the user rather than silently discovered — Simple/Advanced Mode should indicate when a control (Room Size, Wall Material, Tempo/Vari-Speed) only affects Daydream's own player and won't do anything while enhancing another app's audio via System-Wide Mode. Any future advanced audio feature should be scoped with this boundary in mind from the start, rather than building it for the in-app engine and discovering the system-wide gap afterward as happened here.
